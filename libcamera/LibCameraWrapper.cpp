...
static void ensureLibOpened()
{
    if (g_libHandle == NULL) {
        ...
        if (g_libHandle == NULL) {
            ...
            g_getCamSensorBackType = (GetCamSensorBackType) ::dlsym(g_libHandle, "_ZN7android20getCamSensorBackTypeEv");
...
// we need to provide this symbol for libseccameraadapter.so _ZN7android20getCamSensorBackTypeEv

unsigned int getCamSensorBackType(void){
    ensureLibOpened();
    return g_getCamSensorBackType();
}
...
LibCameraWrapper::getParameters() const
{
    ...
    if (mCameraId == 0) {
        ...
        //ret.set("iso-values", "auto,50,100,200,400,800,1600,3200,sports,night");
